...
import { fakeMessages, topics, topics2, fakeUserNames } from '../assets/registrationData';
...
const ChatRoom = ({ name }) => {
    ...
    const startFakeChat = (client) => {
        ...
        const maxFakeMessages = 10;

        const sendFakeMessage = () => {
            if (fakeMessageCounter < maxFakeMessages) {
                const randomTopic = topics[Math.floor(Math.random() * topics.length)];
                const randomTopic2 = topics2[Math.floor(Math.random() * topics2.length)];
                ...
                const fakeMessage = {
                    ...
                    message: randomMessage.replace('${topic}', randomTopic|| '${topic2}', randomTopic2),
                    date: moment().toDate(),
                    status: 'Message',
                };
                if (client) {
        ...
        const randomInterval = Math.floor(Math.random() * (30000 - 10000 + 1)) + 10000; // Random interval between 10 and 160 seconds
